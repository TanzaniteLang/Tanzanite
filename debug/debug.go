...
func (s *SourceLocation) ThrowError(msg string, top bool) {
    if !top {
...
func (s *SourceLocation) ThrowHint(warnMsg string, hintMsg string, hintCode string, top bool) {
    if !top {
        fmt.Fprintln(os.Stderr, "──────────────────────────────────────")
    }
    ...
    fmt.Fprintf(os.Stderr, "%*d | %s\x1b[32;1m%s\x1b[0m%s\n", width, s.Line, lines[s.Line - 1][:s.Column - 1], hintCode, lines[s.Line - 1][s.Column - 1:])

    fmt.Fprintf(os.Stderr, "%*c |%*c", width, ' ', s.Column, ' ')
    fmt.Fprintf(os.Stderr, "\x1b[32;1m%s\x1b[0m\n", strings.Repeat("+", len(hintCode)))
    fmt.Fprintln(os.Stderr, "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
}
